...
-(void)updateVelocity:(CGPoint)point
{
	...
	//angle = angle - CC_DEGREES_TO_RADIANS([[HelloWorldLayer sharedHelloWorldLayer]getRelativeAngle]);
...
- (BOOL)ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event{	
    ...
    //CCLOG(@"converted location.x = %f .y=%f",location.x,location.y);
	location = [self convertToNodeSpace:location];
    //CCLOG(@"converted location.x = %f .y=%f",location.x,location.y);
   
   // location.x+=curParent.position.x;
   // location.y+=curParent.position.y;
